"use client"; // В этом случае use client нужен, потому что корзина и кнопки работают на клиенте.
import { useState, useEffect } from 'react';

// --- ТВОИ НАСТРОЙКИ ИЗ СКРИНШОТОВ И GOOGLE SCRIPT ---
...
export default function Home() {
  ...
  // 1. Загрузка данных через Google Script
  useEffect(() => {
    async function loadData() {
      try {
        ...
        console.error("Ошибка загрузки данных из Google Script:", e);
        // Fallback или сообщение об ошибке, если скрипт не отдает данные
        setProducts([]); // Если ошибка, просто очищаем товары
      } finally {
        setLoading(false);
      }
    }
  ...
  const addToCart = (item) => {
    setCart([...cart, item]);
    // Проверка на доступность Telegram WebApp (если сайт в Telegram)
  ...
  if (loading) return <div style={{background:'#000', color:'#fbbf24', height:'100vh', display:'flex', justifyContent:'center', alignItems:'center', fontSize: '24px', fontWeight: 'bold'}}>ЗАГРУЗКА BOSS STORE...</div>;
  ...
  return (
    <div style={{ backgroundColor: '#000', color: '#fff', minHeight: '100vh', fontFamily: 'sans-serif', paddingBottom: '100px' }}>
      ...
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px', borderBottom: '1px solid rgba(251, 191, 36, 0.2)', position: 'sticky', top: 0, background: 'rgba(0,0,0,0.9)', backdropFilter: 'blur(10px)', zIndex: 10 }}>
        ...
        <button onClick={() => window.open('https://t.me/nmproda')} style={{ background: 'rgba(251, 191, 36, 0.1)', border: '1px solid rgba(251, 191, 36, 0.2)', color: '#fbbf24', padding: '8px 15px', borderRadius: '12px', fontSize: '10px', fontWeight: 'bold', cursor: 'pointer' }}>ПОДДЕРЖКА</button>
      </header>

      {/* КАТЕГОРИИ */}
      <div style={{ display: 'flex', gap: '10px', overflowX: 'auto', padding: '15px 20px', sticky: 'top', top: '70px' }}>
        {categories.map(cat => (
          <button key={cat} onClick={() => setActiveCat(cat)} style={{ padding: '10px 20px', borderRadius: '15px', border: 'none', backgroundColor: activeCat === cat ? '#fbbf24' : '#1a1a1a', color: activeCat === cat ? '#000' : '#fff', whiteSpace: 'nowrap', fontWeight: 'bold', fontSize: '12px', cursor: 'pointer' }}>{cat}</button>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', padding: '20px' }}>
        {products.length === 0 ? (
            ...
            <p>Проверьте Google Таблицу и скрипт.</p>
          </div>
        ) : (
          filtered.map((p, i) => (
            <div key={i} style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '25px', padding: '12px', display: 'flex', flexDirection: 'column' }}>
              <div style={{ width: '100%', aspectRatio: '1/1', background: '#111', borderRadius: '20px', marginBottom: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '30px', color: '#fbbf24' }}>
                {p['Картинка'] && p['Картинка'].startsWith('http') ? (
                  <img src={p['Картинка']} alt={p['Название']} style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: '20px' }} />
                ) : (
                  '📦' // Если картинки нет или ссылка невалидна
                )}
              </div>
      ...
      {/* ПАНЕЛЬ КОРЗИНЫ */}
      {cart.length > 0 && (
        <div style={{ position: 'fixed', bottom: '20px', left: '20px', right: '20px', background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(20px)', border: '1px solid rgba(251, 191, 36, 0.3)', padding: '20px', borderRadius: '30px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 10px 30px rgba(0,0,0,0.5)', zIndex: 100 }}>
          <div>
          ...
          <button onClick={sendOrder} style={{ background: '#fbbf24', color: '#000', padding: '15px 30px', borderRadius: '18px', border: 'none', fontWeight: '900', fontSize: '14px', textTransform: 'uppercase', cursor: 'pointer' }}>ОФОРМИТЬ ({cart.length})</button>
        </div>
      )}
    </div>
  );
